...
export class QuestionsService {
  ...
  async create(dto: CreateQuestionDto): Promise<Question> {
    try {
      ...
      const createQuestionDto = { ...dto };
      delete createQuestionDto.age; // age 는 question 생성때 필요없음.

      const question = await this.repository.save(
        this.repository.create(createQuestionDto),
      );

      await this.repository.manager.query(
        'INSERT IGNORE INTO `question_faction` (questionId, factionId) VALUES (?, ?)',
        [question.id, factionId],
      );
  ...
  // Question 리스트
  async getActives(age = null): Promise<Array<Question>> {
    if (!age) {
    ...
    //   .innerJoinAndSelect('question.factions', 'faction')
  ...
  async getInactives(age = null): Promise<Array<Question>> {
    if (!age) {
      return await this.repository.find({
        relations: ['user'],
        where: {
  ...
  // Question 리스트
  async getActivesBySlug(slug: string, age = null): Promise<Array<Question>> {
    if (!age) {
      return await this.repository.find({
        where: {
          slug: slug,
  ...
  async thumbsUp(id: number): Promise<void> {
    try {
      const question = await this.findById(id, ['user', 'user.profile']);
      await this.repository.increment({ id }, 'up', 1);
      const total = question.up + question.down;
      if (total >= 50) {
        if (question.up < question.down) {
          await this.repository.softDelete(id);
        }
        if (question.up > question.down) {
          await this.repository.manager.query(
            'UPDATE `question` SET isActive = 1 WHERE id = ?',
            [id],
          );
          if (question.isActive == false) {
            //? 작성자에게 2코인 제공 보너스 지급
            const newBalance = question.user.profile?.balance + 2;
            ...
            event.userId = question.userId;
            event.token = question.user.pushToken;
            event.options = question.user.profile?.options ?? {};
